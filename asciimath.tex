...
\def\startParens{\def\useParens##1{\parenWrap{##1}\loopcs}\parseParens}
\def\loopcs{\usePeek\nextToken{%
   \ifx\nextToken`\let\nextC=\finisher
   \else\ifcat\nextToken a\let\nextC=\parseCS
   \else\ifx\nextToken(\let\nextC=\startParens
   \else\let\nextC=\outputAndContinue
   \fi\fi\fi\nextC%
}}
\catcode96=13
\tracingassigns=0
...
%\def\useParens#1{[#1,#1]}\parseParens(xyz)123
`alpha(abc)2`
\tracingmacros=0
\tracingassigns=0

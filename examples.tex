...
engine.  For example, if we type {\tt\char18 (x-1)\char94(2x)/(4-3)\char18} in
the middle of a paragraph, it is typeset as `(x-1)^(2x)/(4-3)`.  
...
\texttt{asciimath}.  (That package provides other useful features such as
deciding whether to write a fraction as `x/y` or `x//y` based on context.)
\begin{itemize}

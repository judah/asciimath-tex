...
Currently there's some typesetting issues, mostly caused by the fact that we always use {\tt
...
\texttt{asciimath}.  (That package also provides other features which may prove
useful, such as choosing between writing a fraction as `x/y` or `x//y`
based on context.)
\begin{itemize}
\item In the first example, the spacing between `f` and the parenthesis is too large;
compare $f(x)$ versus `f(x)`.
\item The parentheses around the inner `sum` are too large; they unnecessarily grow
